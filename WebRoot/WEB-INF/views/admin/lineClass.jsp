  ...
  <body>
		    ...
		    <div class="modal fade" id="addOrEditDialog" tabindex="-1" role="dialog" aria-labelledby="分类编辑" data-backdrop="static">
 			...
 			<small style="float:right">
  			...
  			<form class="form-inline" action="/tour/lineClass/revealObject" method="get">
            ...
            <table class="table table-striped">
              ...
              <tbody>
              <c:forEach var="lineClass" items="${lineClassList}">
		       <tr id="${lineClass.lc_id}">
		       <td><input type="checkbox" class="js-row_selector">${lineClass.lc_id }</td>
		       <td>${lineClass.lc_name }</td>
			   <td>
    ...
    $(document).ready(function () {
	    $('button[name="newEntity"]').click(function (){
	         var url = '/tour/lineClass/view/new';
   	         showModal(url);
	    });
	    
	    $('button[name="editEntity"]').click(function (){
	    	var id = $(this).parents('tr').attr('id');
	         var url = '/tour/lineClass/view/edit?id='+id;
   	         showModal(url);
	    });
          ...
          if (!ids || ids.length <= 0) {
            ...
            bootbox.confirm('此操作将会级联删除其关联的所有数据，确定要删除吗?',function(result){
              if(result) { 
                ...
                var url = '/tour/lineClass/del?' + params;
