  ...
  <body>
		    ...
		    <div class="modal fade" id="addOrEditDialog" tabindex="-1" role="dialog" aria-labelledby="分类编辑" data-backdrop="static">
 			...
 			<small style="float:right">
  			...
  			<form class="form-inline" action="/tour/province/revealObject" method="get">
			  <div class="form-group">
			    <label for="classword">国家分类</label>
            ...
            <table class="table table-striped ">
              ...
              <c:forEach var="province" items="${provinceList}">
		       <tr id="${province.p_id}">
		       <td><input type="checkbox" class="js-row_selector">${province.p_id }</td>
		       <td>${province.country.c_name}</td>
    ...
    $(document).ready(function () {
	    $('button[name="newEntity"]').click(function (){
	         var url = '/tour/province/view/new';
   	         showModal(url);
	    });
	    
	    $('button[name="editEntity"]').click(function (){
	    	var id = $(this).parents('tr').attr('id');
	         var url = '/tour/province/view/edit?id='+id;
   	         showModal(url);
	    });
          ...
          if (!ids || ids.length <= 0) {
            ...
            bootbox.confirm('此操作将会级联删除其关联的所有数据，确定要删除吗?',function(result){
              if(result) { 
                ...
                var url = '/tour/province/del?' + params;
